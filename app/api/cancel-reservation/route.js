...
export async function GET(request) {
  try {
    ...
    // If double slot service (60 min), free up the next slot too
    if (appointment.duration_minutes === 60) {
      const [hours, minutes] = appointment.appointment_time.split(':').map(Number)
      let totalMinutes = hours * 60 + minutes + 30
      const nextHours = Math.floor(totalMinutes / 60)
      const nextMinutes = totalMinutes % 60
      const nextSlotTime = `${nextHours.toString().padStart(2, '0')}:${nextMinutes.toString().padStart(2, '0')}:00`
    ...
    // Send notification email to barber if they have email
                  ...
                  <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 500px; background-color: #000000;">
                    ...
                    <tr>
                      <td style="padding: 20px; text-align: center;">
                        <p style="color: #ef4444; margin: 0; font-size: 48px; line-height: 1;">✕</p>
                        <p style="color: #ef4444; margin: 12px 0 0; font-size: 18px; font-weight: 600;">
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #1a1a1a; border-radius: 12px; overflow: hidden;">
                          ...
                          <tr>
                            <td style="padding: 15px 20px; border-bottom: 1px solid #333;">
                              <p style="color: #888; margin: 0 0 5px; font-size: 12px; text-transform: uppercase;">Klijent</p>
                              <p style="color: #fff; margin: 0; font-size: 16px; font-weight: bold;">${appointment.customer_name}</p>
                            </td>
                          </tr>
                          ...
                          <tr>
                            <td style="padding: 15px 20px; border-bottom: 1px solid #333;">
                              ...
                              <p style="color: #fff; margin: 0; font-size: 16px; font-weight: bold;">${appointment.service_name}</p>
                            </td>
                          </tr>
                    ...
                    <tr>
                      <td style="padding: 20px; text-align: center;">
                        <p style="color: #22c55e; margin: 0; font-size: 14px; font-weight: 600;">
                          ✓ Termin je ponovo slobodan za zakazivanje
                        </p>
                      </td>
                    </tr>
...
function renderHTML(status, message) {
  ...
  return `
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>${title} - Tension Barber</title>
      <style>
    ...
    <body>
      <div class="container">
        <div class="logo">TENSION BARBER</div>
        <div class="icon">${icon}</div>
        <div class="title">${title}</div>
        <div class="message">${message}</div>
        <a href="https://tension-barber.rs" class="button">Nazad na sajt</a>
      </div>
    </body>
    </html>
  `
